...
// conexao
let url = 'mongodb://localhost:27017/FatVot';
...
app.post("/add", async (req, res) => {
    ...
    const I = await new RefDoc({ name: nome , email: email, end: end});
    I.save();
...
// atualizar campos espeficios
  ...
  try {
    const updatedUser = await RefDoc.updateOne(
      { _id: new ObjectId(id) }, // <-- aqui está a mágica
      { $set: update }
    );
    ...
    res.status(500).send({ erro: 'erro no servidor', detalhes: error.message });
  }
});
